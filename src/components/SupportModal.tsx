...
const PRESET_AMOUNTS = [
  { ksh: 100, label: 'KES 100', subtitle: '☕ Coffee' },
  { ksh: 250, label: 'KES 250', subtitle: '🍿 Movie Night' },
  { ksh: 500, label: 'KES 500', subtitle: '⚡ Server Booster', popular: true },
  { ksh: 1000, label: 'KES 1,000', subtitle: '👑 Super Fan' },
];
              ...
              <form onSubmit={handlePaystackCheckout} className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {PRESET_AMOUNTS.map((item) => {
                    const isSelected = !isCustom && selectedAmount === item.ksh;
                    return (
                      <button
                        key={item.ksh}
                        type="button"
                        onClick={() => {
                          setSelectedAmount(item.ksh);
                          setIsCustom(false);
                        }}
